...
export function Profile() {
  ...
  const downloadCertificate = async (ev) => {
    ev.preventDefault();
    ...
    try {
      ...
      downloadLink.download = `document${ext}`;
      downloadLink.click();
        ...
        <div className="flex flex-col gap-8 justify-start min-w-[60%] grow-[1] p-4">
          ...
          <div className="max-w-[52em] flex flex-col w-full rounded-lg border-gray-400 border p-4 gap-4">
            <header className="w-full flex items-center justify-between">
              ...
              <Link to="/dashboard/profile/edit?tab=0">
                <AiFillEdit className="w-8" />
              </Link>
            </header>
            <div className="grow-1 w-full flex flex-wrap justify-center p-[o] gap-2">
              <div className="flex flex-col grow basis-52">
                <label
                  htmlFor="first_name"
                  className="font-thinbold capitalize text-grey"
                  children="first name"
                />
                <span className="mx-1 shrink grow">
                  <input
                    ...
                    type="text"
                    disabled
                    className="rounded-md text-sm p-2 w-full border border-grey"
          ...
          <div className="flex flex-col w-full max-w-[52em] rounded-lg border-gray-400 border p-4">
            <header className="w-full flex items-center justify-between">
              ...
              <Link to="/dashboard/profile/edit?tab=1">
                <AiFillEdit className="w-8" />
              </Link>
            </header>
            {user?.professional_info?.map((item) => (
              <div
                key={item?.id}
                className="grow-1 w-full flex flex-col items-start my-2"
              >
                <h3 className="font-thin text-sm leading-[1em]">
                  {item?.degree}
                </h3>
                <span className="font-thin text-sm leading-[1em]">
                  {item?.school_name}
                </span>
                <span className="font-thin text-sm leading-[1em]">
                  {Moment(item?.start_of_education).format("MMM DD YYYY")} -{" "}
                  {Moment(item?.end_of_education).format("MMM DD YYYY")}
                </span>
                <span className="font-thin text-sm leading-[1em]">
                  {item?.grade}
                </span>
                <p className="leading-[1em] text-md">{item?.study_summary}</p>
              </div>
            ))}
          </div>
          <div className="flex flex-col w-full max-w-[52em] rounded-lg border-gray-400 border p-4">
            <header className="w-full flex items-center justify-between">
              ...
              <Link to="/dashboard/profile/edit?tab=2">
                <AiFillEdit className="w-8" />
              </Link>
            </header>
          ...
          <div className="flex flex-col w-full max-w-[52em] rounded-lg border-gray-400 border p-4">
            <header className="w-full flex items-center justify-between">
              ...
              <Link to="/dashboard/profile/edit?tab=3">
                <AiFillEdit className="w-8" />
              </Link>
            </header>
            <div className="grow-1 w-full flex flex-wrap justify-center p-[o] gap-2">
              <span className="flex-none w-full capitalize text-sm">
                next of kin details
              </span>
              <div className="flex flex-col grow basis-52">
                <label
                ...
                <span className="mx-1 shrink grow">
                  <input
                    ...
                    disabled
                    className="rounded-md text-sm p-2 w-full border border-grey"
                    value={user?.contact_info?.next_kin_fname}
                  />
                </span>
              </div>
              <div className="flex flex-col grow basis-52">
                ...
                <span className="mx-1 shrink grow">
                  <input
                    ...
                    disabled
                    className="rounded-md text-sm p-2 w-full border border-grey"
                    value={user?.contact_info?.next_kin_lname}
                  />
                </span>
              </div>
              <div className="flex flex-col grow basis-52">
                ...
                <span className="mx-1 shrink grow">
                  <input
                    ...
                    disabled
                    className="rounded-md text-sm p-2 w-full border border-grey"
                    value={user?.contact_info?.next_kin_phone}
                  />
                </span>
              </div>
            </div>
                ...
                <span className="mx-1 shrink">
                  {user?.professional_cert?.certificate ? (
                    <a
                      ...
                      href={user?.professional_cert?.certificate}
                      className="bg-blue px-6 py-2 text-white uppercase text-sm rounded"
                      dis
                    >
                      download
                    </a>
