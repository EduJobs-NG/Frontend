...
export const SearchJobs = ({setShowSearchJobs, setShowRecentJobs}) => {
  ...
  return (
    ...
    <div className='container mx-auto py-[4rem]'>
      ...
      <div className='grid md:grid-cols-2 gap-3 md:gap-[6rem] place-items-center'>

      <div className='bg-white hover:border-[#fff]  transition-all px-[1rem] py-[0.3rem] w-full  border rounded-[5px] border-[#808080] flex flex-row justify-start items-center gap-2 '>
      ...
      <div className='bg-white hover:border-[#fff]  transition-all px-[1rem] py-[0.3rem] w-full  border rounded-[5px] border-[#808080] flex flex-row justify-start items-center gap-2 '>
        ...
        <div className='bg-[#000] ml-[0.5rem]  h-[2rem] w-[3px]'></div>
        <div className=''>
        <FormInputBox placeholder='e.g., Lagos'
        value={searchKeyword}
        onChange={(e) => setSearchKeyword(e.target.value)}
         className="border-none p-2.5 block   w-full border-[#808080] outline-none" />
        </div>
      ...
      <div className='grid mt-[2rem] md:mt-[3rem]   place-items-center'>
           {!isLoading && <button onClick={handleJobSearch} className='bg-[#f0f0f0] text-blue hover:text-white hover:bg-blue uppercase opacity-100 w-full md:w-[300px] px-[5rem] rounded-[5px] p-4' type="submit">Search Jobs</button> }
